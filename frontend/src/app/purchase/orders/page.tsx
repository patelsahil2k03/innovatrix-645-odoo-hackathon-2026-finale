...
import { api, type AnalyticAccount, type Contact, type Product, type PurchaseOrder, type PurchaseOrderUpdate } from "@/lib/api";
...
interface PurchaseOrderEditFormProps {
  ...
  onSave: (values: PurchaseOrderUpdate) => Promise<void>;
}
...
function PurchaseOrderEditForm({ order, vendors, products, analyticAccounts, onSave }: PurchaseOrderEditFormProps) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await onSave({ vendor_id: vendorId, order_date: orderDate, reference: reference || null, lines: linesResult.data });
    } catch (error) {
...
function PurchaseOrdersPageInner() {
  ...
  const statusFilter = useStatusFilter("purchase_orders");
  ...
  const orders = useFetch(
    () => api.purchaseOrders.list({ page, page_size: PAGE_SIZE, q: debouncedSearch, sort: sort ?? undefined, status: statusFilter.status ?? undefined }),
  ...
  return (
    <AppShell>
      ...
      <StatusChips
        ...
        aria-label="Purchase orders by state"
      />
            ...
            <>
              ...
              {data.status === "DRAFT" && canRecord ? (
                <PurchaseOrderEditForm
                  ...
                  analyticAccounts={analyticAccounts.data?.items ?? []}
                  onSave={async (values) => {
                    await api.purchaseOrders.update(data.id, values);
